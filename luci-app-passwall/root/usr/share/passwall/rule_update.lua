...
local function GeoToRule(rule_name, rule_type, out_path)
	...
	if not (bin and api.compare_versions(api.get_app_version("geoview"), ">=", "0.1.10")) then
		log("[warn] Geoview Component is missing or the version is too low，Rule generation process has been skipped。")
		return false
	end
	...
	if local_file_size == 0 then
		os.remove(out_path)
		log(rule_name .. " Build failed，please make sure Geo File is correct and contains target rules。")
		return false
	end
	return true
end
...
local function check_instance(action)
	...
	if action == "start" then
		...
		if fs.access(rule_lock) then
			log("have[Rule updates]Instance is running，Please try again later...\n")
			os.exit(0)
		else
	...
	if fs.access(sub_lock) then
		log("[subscription]Instance is running，[Rule updates]Enter the queue and wait...\n")
	end
